...
export const TextGenerator = ({nodes, edges, setNodeLabels, setErrorMessage, setNextGroup, selectedLanguage}) =>{
    ...
    const generateText = () => {
        ...
        // Group edges by conID of the source node
        edges.forEach(edge => {
        ...
        const dfs = (node, visited, group) => {
            visited.add(node.id);
            group.nodes.push(node);
    
            const connectedEdges = group.edges.filter(edge => edge.source === node.id);
    
            connectedEdges.forEach(edge => {
                const targetNode = nodes.find(n => n.id === edge.target);
                if (targetNode && !visited.has(targetNode.id)) {
                    dfs(targetNode, visited, group);
                }
            });
        };
        ...
        conIDGroups.forEach((group, conID) => {
            ...
            group.nodes.forEach(node => {
                if (!visited.has(node.id)) {
                    dfs(node, visited, group); // Start DFS from this node
                }
            });
